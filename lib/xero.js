...
// What each scope actually buys. preflight reads the granted scopes off the
// stored grant and reports against this, so a missing one is named together
// with the feature it takes away rather than left as a string to decode.
// The four sign-in scopes are omitted: a grant that lacks them never works at
// all, so there is nothing useful to say about them.
const REQUIRED_SCOPES = [
  ['accounting.invoices', 'bills, submit and approve, and both sides of a recharge'],
  ['accounting.contacts', 'payee details, and the contact a recharge creates'],
  ['accounting.settings.read', 'the bank accounts a payment can be made from'],
  ['accounting.payments', 'recording a bank-file batch as a payment in Xero']
];

// Which of those a grant does NOT cover, given the scope string Xero recorded at
// consent. Apps predating March 2026 hold the broad accounting.transactions,
// which subsumes the three accounting scopes it was split into — but not
// settings.read, which was always separate. Returns [] for a grant that records
// no scope at all: absent evidence is not evidence of absence, and preflight
...
function missingScopes(scopeString) {
  ...
  const broad = granted.has('accounting.transactions');
  return REQUIRED_SCOPES.filter(([scope]) => {
    if (granted.has(scope)) return false;
    if (scope === 'accounting.settings.read') return !granted.has('accounting.settings');
    return !broad;
  });
}
...
async function accessTokenFor(accountId, tenantId) {
  ...
  if (!inflight) {
    // What the refresh actually spent, so a failure can be told apart from a
    // rotation that happened while we held it.
    let attemptedWith = null;
    const refreshOnce = () => xc.withGrantLock(grant.grantId, async (currentRefreshToken) => {
      attemptedWith = currentRefreshToken;
      ...
      if (!res.ok) {
        ...
        err.statusCode = 401;
        // Xero's own error code. The description is prose and varies; this does
        // not, and telling a spent token from a bad secret depends on it.
        err.xeroError = payload.error || null;
        throw err;
      }
    ...
    inflight = (async () => {
      try {
        ...
        // Xero refresh tokens are single-use. Bills Hub locks the row, but
        // WazzOCR's refresh path does not, so it can spend the token between
        // our read and our use of it. That looks identical to a dead grant —
        // except that the stored token will have changed. If it has, the
        // rotation is the explanation and the new token works; try once more.
        // If it has not, the grant really is gone and the error stands.
        if (err.xeroError !== 'invalid_grant') throw err;
        // WazzOCR's own UPDATE was blocked on the row lock we just released, so
        // give it a moment to land before deciding the token is unchanged.
        await sleep(300);
        const again = await xc.getGrantForTenant(wazzocrAccountId, tenantId).catch(() => null);
        if (!again || !attemptedWith || again.refreshToken === attemptedWith) throw err;
        console.warn('[xero] the refresh token rotated underneath us — retrying with the current one.');
        return refreshOnce();
      }
...
} else if (grantSource.BORROWED) {
  // Own mode reaches only what this app was consented for, which during testing
  // is a demo company. A borrowed grant reaches all 41 live organisations, so an
  // empty allowlist there is the difference between a test deployment and one
  // that can approve and pay real bills.
  console.warn('');
  console.warn('  ****************************************************************');
  console.warn('  *  XERO_GRANT_SOURCE=wazzocr with no XERO_TENANT_ALLOWLIST.    *');
  console.warn('  *  Writes are permitted to EVERY organisation on the borrowed  *');
  console.warn('  *  grant, including live ones. Set the allowlist while you are *');
  console.warn('  *  still testing:  npm run entities only <CODE>                *');
  console.warn('  ****************************************************************');
  console.warn('');
}
...
// Xero's 60 calls/minute is per tenant per APP — and on a borrowed grant
// WazzOCR is the same app. The two share one budget, so taking 55 of 60 would
// leave WazzOCR four calls a minute and push its live pipeline into 429s during
// a backfill. Default to a minority share there and let the operator raise it
// once nothing else is using the grant.
const DEFAULT_CALLS_PER_MIN = grantSource.BORROWED ? 25 : 55;
const CALLS_PER_MIN = Number(process.env.XERO_CALLS_PER_MIN || DEFAULT_CALLS_PER_MIN);
if (grantSource.BORROWED && !process.env.XERO_CALLS_PER_MIN) {
  console.log(`[xero] sharing a borrowed grant — capping at ${CALLS_PER_MIN} calls/min per organisation, leaving the rest for WazzOCR.`);
}
...
module.exports = {
  ...
  SCOPES_REQUIRED: REQUIRED_SCOPES, missingScopes,
